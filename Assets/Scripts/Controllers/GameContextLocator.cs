    ...
    internal sealed class GameContextLocator
    {
        ...
        internal void Register<T>(T controller) where T : IController
        {
            ...
            if (_controllers.ContainsKey(key))
                Debug.LogError($"Attempted to register controller of type: {key} which already register with the {GetType().Name}.");
            else
        ...
        internal void Unregister<T>() where T : IController
        {
            var key = typeof(T).Name;
            if (!_controllers.ContainsKey(key))
                Debug.LogError($"Attempted to unregister controller of type: {key} " +
